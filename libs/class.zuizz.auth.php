...
class ZUAUTH implements ZUAUTH_interface
{
    ...
    public function login($username, $credentials)
    {
        if (isset ($username) && isset ($credentials) && !empty($username)) {
            $tmp = ORM::for_table('org_user')->where('name', $username)->find_one();
            if ($tmp) {
                $d = $tmp->as_array();

                if ($username == $d['name'] && $d['password'] == md5($GLOBALS ['ZUIZZ']->config->system ['salt'] . $credentials . $GLOBALS ['ZUIZZ']->config->system ['salt'])) {
                    $_SESSION ['ZUIZZ'] ['AUTH'] ['is_auth'] = TRUE;
                    $_SESSION ['ZUIZZ'] ['AUTH'] ['username'] = $username;
                    $_SESSION ['ZUIZZ'] ['AUTH'] ['uid'] = $d['user_id'];
                    $_SESSION ['ZUIZZ'] ['AUTH'] ['contact'] = $d['contact_id'];
                    $_SESSION ['ZUIZZ'] ['AUTH'] ['org'] = $d['org_id'];
